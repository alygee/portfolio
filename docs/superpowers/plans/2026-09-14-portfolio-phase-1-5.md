...
> **Пометка по факту (Task 4):** ESLint не встал — `typescript-eslint` во всех версиях, включая canary, требует `typescript <6.1.0`, а проект закреплён на `typescript@7.0.2`. Понижать TypeScript ради линтера не стали. Линтером в итоге стал **Oxlint 1.83.0** (не зависит от пакета `typescript` как peer). Цель и правила те же — recommended JS/TS, `react-hooks`, запрет импорта `three` вне сцены. Подробности: `.superpowers/sdd/2026-09-14-portfolio-phase-1-5/task-4-report.md`.
...
> **Пометка по факту:** реализовано на **Oxlint**, не ESLint — см. пометку у Tech Stack выше и `.superpowers/sdd/2026-09-14-portfolio-phase-1-5/task-4-report.md`. Ниже сохранён исходный бриф как есть, для истории.
