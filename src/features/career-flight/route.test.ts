...
import { LOOK_AHEAD, cameraWaypoint, progressToCamera, route, stationPosition } from './route';
...
describe('привязка камеры к станциям', () => {
  it('камера останавливается рядом со станцией в её прогрессе', () => {
    ...
    // дуги (сама формула проверена в `entities/route/progress.test.ts`).
    // Этот тест не проверяет формулу, а закрепляет инвариант, важный для
    // следующих задач: камера должна оказываться достаточно близко к путевой
    // точке каждой станции, когда прогресс равен stationProgress этой
    // станции. Если геометрия маршрута изменится так, что камера перестанет
    // останавливаться у станций, тест упадёт независимо от того, как именно
    // вычисляется stationProgress.
    for (let i = 0; i < COUNT; i += 1) {
